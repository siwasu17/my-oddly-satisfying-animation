...
**出力された PNG を自分で `Read` しない。** 画像 1 枚が文脈に約 1,000 トークン積み上がる。
`Explore` エージェント 1 体に渡して、**講評テキストだけ**を受け取る。

```
Agent({ subagent_type: "Explore", description: "スクショの講評", prompt: ... })
```

プロンプトはこの骨子で渡す（**順番が要点**）:

> 1. まず `<出力された PNG の絶対パス>` を `Read` して、**この画像が何に見えるかを一言**で
>    答えてください。この時点では下の 2 を読まないでください。
> 2. そのあとで読んでください — このシーンは「〈狙いを 1 行で〉」を狙っています。
>    これを踏まえて、`.claude/skills/new-scene/references/recipes.md` の
>    **「見え方のセルフチェック」の 6 項目**に ○ / × で答えてください。
> 3. × を付けた項目ごとに、何をどちらへ振れば直るかを 1 行で提案してください。
>
> **修正しないでください。コマンドも実行しないでください。`Read` だけです。**

**1 を先に言わせるのが肝心。** 狙いを先に見せると意図を投影して見てしまい、
1 項目目のチェックが機能しない。予備知識ゼロの目で第一印象を言わせるこの順番なら、
エージェントがコールドスタートであることが**判定として正しく働く**。
狙いを知っている自分には原理的にできない検査なので、これは節約であると同時に品質の担保でもある。
