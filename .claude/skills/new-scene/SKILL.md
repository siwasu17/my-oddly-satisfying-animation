...
**この場合は棚卸しエージェントを呼ばない。** 下の Explore が要るのは案を出すときだけ。
コンセプトが決まっているのに投げるのは丸損になる。

### コンセプトが無い場合

**既存シーンの棚卸しは `Explore` エージェント 1 体に任せる。自分で `head -20` を全ファイルに
叩かない。** シーンは 20 本を超えていて、冒頭コメントを全部読むと 7K 以上を消費する。
しかもシーンが増えるたびに増え続ける。返してもらうのは案だけでよい。

```
Agent({ subagent_type: "Explore", description: "既存シーンの棚卸し", prompt: ... })
```

プロンプトはこの骨子で渡す:

> `src/scenes/*.ts`（`index.ts` は除く）の冒頭コメントと `SceneModule` の `name` / `desc` を読み、
> 次の 2 つを返してください。
>
> 1. 既存シーンの「動きの質」を 1 行ずつ列挙する（何が、どんな質感で動くか）
> 2. それらと動きの質がかぶらない新シーン案を 3〜4 個。各案に
>    「何が動くか / 気持ちよさの芯 / 音は何を鳴らすか」を 1 行ずつ添える
>
> 案は Three.js のプリミティブだけで作れて、1 画面で完結し、永久にループするものに限ります。
> 外部アセットを読み込む案、依存の追加が要る案は出さないでください。
>
> **ファイルを変更しないでください。コマンドも実行しないでください。読むだけです。**

返ってきた案をそのまま `AskUserQuestion` の選択肢にして提案する。
エージェントのレポートはユーザーには見えないので、案の中身は自分の言葉で選択肢に写すこと。
...
**`shot` は自分で実行する。** 終了コードとパスが要る。非ゼロで落ちたらページ内 JS エラーなので、
講評エージェントは呼ばずにそのまま原因を直す。

#### 撮れた画像は講評エージェントに見せる

**出力された PNG を自分で `Read` しない。** 狙いを知っている目では下の 1 項目目
（意図した被写体だと一目で分かるか）が原理的に検査できない。画像 1 枚が文脈に
約 1,000 トークン積み上がるのも避けられる。`scene-critic` エージェント 1 体に渡して、
**講評テキストだけ**を受け取る。

```
Agent({ subagent_type: "scene-critic", description: "スクショの講評", prompt: ... })
```
...
手順（第一印象を先に言わせてから 6 項目を判定する順番）も、出力の形も、
**エージェント定義 `.claude/agents/scene-critic.md` が持っている。ここに写さない。**
2 か所に置くと必ず食い違う。

> **`scene-critic` が見つからないと言われたら**、エージェント定義はセッション開始時に
> 読まれるので、その定義より前に始まったセッションにいる。代わりに `claude` エージェントへ
> 上の 2 つに加えて「`.claude/agents/scene-critic.md` を `Read` して、その手順どおりに
> 講評してください」と渡す。**`Explore` で代用しない**（抜粋を読む探索用で、レビューはしない
> と定義に明記されている。講評の質がこのステップの目的そのものなので代役にならない）。

**狙いの説明を長々と書かない。** 1 行に収める。エージェントは第一印象を答えるまで狙いを
読まない作りになっているが、プロンプトが長いほどこの順番は壊れやすくなる。
予備知識ゼロの目で見てもらうことが、このステップの value そのものなので、
**コンテキストを渡さないことが品質の担保**になっている（節約はついでに過ぎない）。

× が付いた項目は**まとめて**直し（定数ブロックを `Edit` 1 回で振る）、それから 1 枚撮り直す。
1 つずつ当てずっぽうに撮り直さない。**撮るのは 5 枚まで**（発散を防ぐため）。
5 枚で決まらないときは、細部を詰めずに Phase 8 でユーザーへ渡して判断を仰ぐ。

2 枚目以降は **`SendMessage` で同じエージェントに次のパスを渡す。** 前の画像を覚えているので
「前より良くなったか」を判定でき、spawn し直しのコストも要らない。

講評の要点（第一印象と × が付いた項目）は Phase 8 の報告に載せる。
エージェントのレポートはユーザーには見えない。
...
4. 検証結果（typecheck / build / smoke がすべて通ったこと）と、
   **講評エージェントの第一印象・× が付いた項目**（ユーザーはそのレポートを見ていない）
