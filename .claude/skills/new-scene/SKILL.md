...
**実装前に `references/recipes.md` を読む。** InstancedMesh・色・音・カメラ・乱数の定型が載っている。
毎回ゼロから書かない。
...
さらに、**見た目を必ず一度は自分で確認する**。`npm run play -- --bg <scene-name>` で起動し、
`agent-browser` で URL を開いてスクリーンショットを撮る（タブの末尾に自分のシーンがある）。
失敗したら直して再実行する。型エラーを `any` や `!` で黙らせない。
