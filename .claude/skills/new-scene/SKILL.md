...
`.claude/settings.json` の `worktree.baseRef: "head"` を当てにしない。**設定はセッション
起動時に読まれるので、効いているとは限らない。** 上の確認は毎回行うこと。
...
- worktree に入ったら、**このセッションのまま Phase 3 へ進む。**
...
コミットメッセージは日本語で、1 行サマリ + 仕様の要点。
末尾の `Co-Authored-By:` 行は、**そのセッションに指示されている行をそのまま写す**
（モデル名を書き固めない。セッションによって違う）。
