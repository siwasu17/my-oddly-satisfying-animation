...
既存のシーンから抜き出した「毎回書くもの」。ゼロから考えず、まずここを見る。
...
**出力された PNG を自分で `Read` しない。** `scene-critic` エージェントに渡して講評だけ受け取る
（手順は `../SKILL.md` の Phase 6「撮れた画像は講評エージェントに見せる」）。
画像 1 枚が文脈に約 1,000 トークン積み上がるうえ、狙いを知っている目では
下の 1 項目目が原理的に検査できない。
...
**この 6 項目がチェックの正本。** 講評エージェントにはここを読ませる（SKILL.md 側に写さない。
2 か所に置くと必ず食い違う）。**迷ったら × を付ける。**
6 項目には、過去に実際にユーザーから指摘された内容がそのまま並んでいる。
...
□ 色が暖色帯に収まっているか（青い光が混じっていないか）
```

ページ内 JS エラーはこの一覧に無い。**`npm run shot` の終了コードが判定する**ので、
画像を見て探す必要は無い（非ゼロで落ちた時点で、講評へ進む前に直す）。

**撮るのは 5 枚まで**（発散を防ぐため）。
5 枚で決まらないときは、細部を詰めずにユーザーへ渡して判断を仰ぐほうが早い。

調整は**当てずっぽうで 1 つずつ撮り直さない**。× が付いた項目を全部拾って
