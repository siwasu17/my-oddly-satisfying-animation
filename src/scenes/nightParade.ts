 ...
 * 百鬼夜行。
 *
 * 何が動くか: 闇に浮かぶ環状の夜道を、妖怪の行列がひたすら漂い巡る。
 ...
 *
 * 動き方は「歩く」ではなく「漂う」。足が無いものが地面に接地していると、
 * それだけで歩いて見えてしまうので、道の面から 0.45 浮かせてある。
 * 上下の揺れにはかつて |sin| を使っていたが、あれは底で折れ線になり、
 * 最大加速度が 100 単位/秒² を超える。数値上の跳ねがそのまま
 * 「足で地面を蹴っている」というぎこちなさになっていた。
 * いまは速さの違う正弦を 2 つ重ねている（最大加速度 0.2 単位/秒²）。
 * 1 つだけだと折り返しが等間隔に来て、機械の往復に見える。
 * 横の漂いは上下の 0.618 倍（黄金比の逆数）で、2 つは永遠に揃わない。
 * 傾きは横の「速さ」に比例させてあるので、流れる向きへ身体が倒れ、
 * 折り返しでいちばん立つ。布と笠の揺れは、それよりさらに遅い。
 *
 * ただし「そこに在る物」としては描かない。物として立たせると、行列が
 * 夜道を来る絵が生々しくなりすぎて、寝る前に眺めるものではなくなる。
...
/**
 * 妖怪ごとに持つ値の数。
 * [道の位置, 道幅方向のずれ, 背丈, 布の揺れの速さ, 位相, 横幅の倍率,
 *  漂う向き, 型, シーツの明度, 浮き沈みの速さ, 第二位相]
 */
const STRIDE = 11;
 ...
 * 道の面からどれだけ浮いているか。
 *
 * 足が無いものを地面に接地させると、それだけで「歩いている」に見えてしまう。
 * 裾のいちばん低いところ（波の谷）が道に触れない高さまで持ち上げる。
 * 浮き沈みの振れ幅（0.29）を引いても裾が道へ潜らない値がここ。
 */
const HOVER = 0.45;
/**
 * 横へ漂う幅。道幅（±1.7）の中で行ったり来たりする。
 * 広げすぎると列が崩れて、行列ではなく散開に見える。
 */
const DRIFT_W = 0.5;
...
function buildGate(): THREE.Group {
   ...
   * 道から浮いているぶん（0.45）と浮き沈みの上限（0.29）を足して約 2.6。
   * 上端のゆらぎを足しても 2.8 には届かない。貫をその上（2.95）へ置くと、
...
/** 霞にほどけかけた笠とシーツのおばけが、うねる夜道をどこまでも漂っていく。 */
export const nightParade: SceneModule = {
  name: 'Night Parade',
  desc: '闇の環を漂い巡る笠をかぶったシーツおばけに、人魂が尾を引いて寄り添う。',
  ...
  build(root) {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      // 布の揺れの速さ。歩調だったころは 5.0〜7.8（毎秒 1 歩ほど）だった。
      // そのままだと布がぱたぱた鳴って、浮いているようには見えない
      oni[o + 3] = 1.2 + rnd() * 0.8;
      oni[o + 4] = rnd() * TAU;
      // 背丈と横幅を逆に振る。背の高いものは痩せ、低いものは横に広がる。
      // 広げすぎると、背の低い個体の幅が丈に並んで団子に見える
      ...
      // 浮き沈みの速さ。布の揺れよりさらに遅い。ここが速いと跳ねて見える
      oni[o + 9] = 0.42 + rnd() * 0.38;
      // 第二位相。浮き沈みと横の漂いをずらすのに使う
      oni[o + 10] = rnd() * TAU;
  ...
  update(t) {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const wide = oni[o + 5]!;
      const hand = oni[o + 6]!;
      // 布の揺れ。シェーダ側の aGait と同じ値から作る
      const gait = t * oni[o + 3]! + oni[o + 4]!;
      // 浮き沈みの位相と、それより遅い横漂いの位相。
      // 比を 0.618（黄金比の逆数）にしてあるので 2 つは永遠に揃わず、
      // 「上がりながら右へ」「沈みながら左へ」の組み合わせが毎回変わる
      const rise = t * oni[o + 9]! + oni[o + 4]!;
      const swayRate = oni[o + 9]! * 0.618;
      const sway = t * swayRate + oni[o + 10]!;

      // 前後のゆらぎ。全員が同じ速さで進むと、等間隔の行進に見える。
      // 一周ぶんの 0.6% ほど（弧にして 0.5 弱）だけ前後させると、
      // 詰まったり離れたりしながら、列としては崩れない
      const u = head + oni[o]! + 0.006 * Math.sin(rise * 0.8 + oni[o + 10]!);
      roadAt(u, here);
      roadAt(u + DU, ahead);
      const dx = ahead.x - here.x;
      const dz = ahead.z - here.z;
      ...
      // 浮き沈み。速さの違う正弦を 2 つ重ねる（比は無理数寄り）。
      // 1 つだけだと上下の折り返しが等間隔に来て、機械の往復に見える。
      // かつては |sin| で跳ねさせていたが、あれは底で折れ線になるので、
      // 足で地面を蹴っているように見えてしまっていた
      const lift = HOVER + 0.2 * Math.sin(rise) + 0.09 * Math.sin(rise * 1.61 + 1.3);

      // 横の漂い。道幅の中をゆっくり行き来する
      const across = DRIFT_W * Math.sin(sway) * hand;
      const side = oni[o + 1]! + across;
      const x = here.x + nx * side;
      const y = here.y + lift;
      const z = here.z + nz * side;

      // 向き。進行方向を向いたまま、ゆっくり首を振る
      const yaw = Math.atan2(tx, tz) + 0.14 * Math.sin(sway * 0.7 + oni[o + 4]!);
      // 前後の傾きは浮き沈みに遅れて付く。沈むときに前へ、浮くときに後ろへ
      const lean = -0.03 - 0.07 * Math.cos(rise);
      // 横の傾きは漂う速さに比例させる。流れる向きへ身体が倒れるので、
      // 折り返しでいちばん立ち、いちばん速いところでいちばん傾く
      const roll = -DRIFT_W * swayRate * Math.cos(sway) * hand;

      // シーツ。丈と太さを別々に掛けるので、痩せた背高と低い横広が混ざる
      dummy.position.set(x, y, z);
      dummy.rotation.set(
        lean, // 沈むときに前へ、浮くときに後ろへ
        yaw,
      ...
      // シーツは闇に近い色。個体ごとのオフセットに、揺れぶんの明暗を足す。
      ...
      // 位置だけ別に組み立てると、漂うたびに笠が頭から外れて宙に残る
      perch.set(
