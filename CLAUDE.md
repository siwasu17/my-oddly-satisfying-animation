...
タブは**新しいシーンほど先頭**に並ぶ。「新しさ」は **git がそのファイルを追加した時刻**で、
`vite.config.ts` のプラグインがビルド時に読んで渡している（`scripts/scene-order.mjs`）。
まだコミットしていないシーンは最新扱いで先頭に出る。
**並び順を手で管理する場所は無い**（以前あった `ORDER` という手書きの表は廃止した）。
...
- 1000 行を大きく超えそうなら、動きを減らすほうを先に考える。
